...
-- | 'Image' is an array of Floats in two dimensions
type Image	= Array U DIM2 Float

-- | 'gradientX' horizontal sobel edge stencil template
...
-- | 'gradientY' vertical sobel edge stencil emplate
...
-- | to calculate modulus  of a vector
modI :: Float -> Float -> Float 

modI x y
        = sqrt (x * x + y * y)

-- | 'go' is the helper function to run.All the iterations are carried out through it.
