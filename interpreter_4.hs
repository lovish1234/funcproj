...
foo xs = 
    forever $ do
        ...
        print x
        --print y
        if ((take 5 x) == "Flood") then (fillColor (read x :: Commands)) else (outputBMP [(read x :: Picture)] imageData1)
        ...
        let     
            ...
            image = Byte.concat [Byte.pack header, Byte.pack imageData]
                in do
                    ...
                    --print ([foo1 y 100 400 200 200 | y <- [0..158]]) 
                    --print imageData                  
...
foo3 xs imageData
	| (Flood y x) <- xs
	= if ((imageData !! (ceiling (x*568 + y)*3)) == 0 || (imageData !! ((ceiling (x*568 + y)*3)+1)) == 0 || (imageData !! (ceiling (x*568 + y)*3+2) == 0)) then [] else concat[[(x,y)], foo3 (Flood (y) (x-1)) imageData, foo3 (Flood (y) (x+1)) imageData, foo3 (Flood (y+1) (x-1)) imageData,foo3 (Flood (y+1) (x)) imageData,foo3 (Flood (y+1) (x+1)) imageData,foo3 (Flood (y-1) (x-1)) imageData,foo3 (Flood (y-1) (x)) imageData,foo3 (Flood (y-1) (x+1)) imageData]
    ...
    =  do
        ...
        let 
            ...
            in do
				...
				Byte.writeFile "temp.bmp" image
                {-in
                 if (imageData1 !! n2 == 0) then (Byte.writeFile "temp_out_1.bmp" image) 
                 else do
                    Byte.writeFile "temp_out_1.bmp" image
                    fillColor (Flood (y+1) (x+1))
                    fillColor (Flood (y+1) (x))
                    fillColor (Flood (y+1) (x-1))
                    fillColor (Flood (y) (x+1))
                    fillColor (Flood (y) (x-1))
                    fillColor (Flood (y-1) (x+1))
                    fillColor (Flood (y-1) (x))
                    fillColor (Flood (y-1) (x-1))
